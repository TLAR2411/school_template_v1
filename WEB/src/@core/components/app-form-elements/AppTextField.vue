...
// Helper: Convert Khmer digits to English digits
const convertKhmerToEnglish = (value) => {
  return String(value).replace(/[០១២៣៤៥៦៧៨៩]/g, (char) => {
    return char.charCodeAt(0) - 0x17e0;
  });
};
...
const handleInput = (event) => {
  let inputValue = convertKhmerToEnglish(event.target.value);
...
const handleKeypress = (event) => {
 const char = convertKhmerToEnglish(String.fromCharCode(event.keyCode || event.which)); // 👈 convert first
