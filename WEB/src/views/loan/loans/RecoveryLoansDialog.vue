      ...
      <VCard>
        ...
        <div ref="target" data-capture-root="true" class="pa-2">
          ...
          >
            ...
            <span class="mt-1 mb-1 text-center" style="font-size: 15px">
             វគ្គទី {{ itemData.loan_cycle }}
            </span>
            ...
            <VDivider class="mt-1 mb-2" />
            <div class="d-flex flex-row mt-1 mb-1 justify-space-between">
              <div class="text-primary">ទំហំកម្ចី</div>
              <div>
                {{ formatNoneZero(itemData.loan_amount) }}
              </div>
            </div>
            <VDivider class="mt-1 mb-2" />
            <div class="d-flex flex-row mt-1 mb-1 justify-space-between">
              <div class="text-primary">ស្ថានភាពកម្ចី</div>
              <div>
                <VChip
                  color="success"
                  size="small"
                  v-if="itemData.loan_type == 'current'"
                >
                  {{ $t("Current Loan") }}
                </VChip>
                <VChip
                ...
                <VChip
                  color="error"
                  size="small"
                  v-if="itemData.loan_type == 'overdue'"
                >
                  {{ $t("Overdue Loan") }}
                </VChip>
              </div>
            ...
            <VDivider class="mt-1 mb-2" />
            <div class="d-flex flex-row mt-1 mb-1 justify-space-between">
              <div class="text-primary">សរុបប្រាក់ត្រូវបង់ផ្ដាច់</div>
              <div>
                {{
                  formatNoneZero(
                    parseFloat(itemData.principal_balance) +
                      parseFloat(itemData.interest_balance) +
                      parseFloat(itemData.late_penalty_amount || 0)
                  )
                }}
              </div>
            </div>
