...
// Address codes come back from the API as strings; the pickers expect numbers
  ...
  try {
    ...
    if (res.data.status) {
      ...
      formData.value = {
        ...
        b_province_code: toCode(data.b_province_code),
        b_district_code: toCode(data.b_district_code),
        b_commune_code: toCode(data.b_commune_code),
        b_village_code: toCode(data.b_village_code),
      };
    }
  } catch (error) {
