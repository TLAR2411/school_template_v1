...
const showFilters = ref(
  savedOptionsRight?.showFilter || (props?.showFilters ? [0] : []),
);
...
const selectedItem = ref(
  savedOptionsRight?.selectedItem || props.selectedItem || null,
);
  ...
  <VLayout class="chat-app-layout h-100" style="z-index: 0" :class="border">
    <VMain class="chat-content-container">
      <div class="d-flex flex-column h-100">
        <div
          class="active-chat-header d-flex align-center text-medium-emphasis bg-surface pl-6 pr-3 pt-2 pb-2"
        >
          <div class="d-flex align-center">
            <div
              class="d-flex flex-column text-primary"
              v-if="selectedItemHeader?.title"
            >
              ...
              <div
                v-if="selectedItemHeader?.objectList?.[0]?.value"
                class="d-flex mt-1"
              >
                <VChip class="mr-2" size="small">
                  <span class="text-secondary" style="font-size: 13px"
                    >{{
                      $t(selectedItemHeader.objectList[0].title)
                    }}:&nbsp;</span
                  >
                  <span class="text-secondary" style="font-size: 13px">{{
                    selectedItemHeader.objectList[0].value
                  }}</span>
                </VChip>
              </div>
            </div>
